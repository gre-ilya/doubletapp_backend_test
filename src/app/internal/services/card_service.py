...
class CardService:
    ...
    async def create_card(user_id, first_name, last_name):
        ...
        card_to_save = Card(
            id=card_id, account=db_bank_account, expire_month=2, expire_year=28, owner_name="vasya", cvv="123"
        )
    ...
    async def get_card(user_id):
        ...
        db_bank_account_data = await BankAccountService.get_bank_account(user_id)
