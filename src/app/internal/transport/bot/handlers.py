...
NO_BANK_ACCOUNT_RESPONSE = "You have to create your bank account with " \
                           "/open_bank_account before using this command."
NO_CARD_RESPONSE = "You have to create your card with /issue_bank_card " \
                   "before using this command."
...
async def me_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_entity = UserEntity.create_from_db_data(
        await UserService.get_user(user_entity.id))
...
async def account_info_handler(update: Update,
    ...
    if not await Checker.user_has_bank_account(user_entity):
        return await context.bot.send_message(chat_id=update.effective_chat.id,
                                              text=NO_BANK_ACCOUNT_RESPONSE)
    # Response to user
    account_entity = BankAccountEntity.create_from_db_data(
...
async def issue_bank_card_handler(update: Update,
                                  context: ContextTypes.DEFAULT_TYPE):
    """This handler updates user data, checks if this command is available to
    ...
    if await Checker.has_card(user_entity):
        text_back = "You already have a card. Use /card_info to check your " \
                    "card information."
        return await context.bot.send_message(chat_id=update.effective_chat.id,
                                              text=text_back)
    # Create card
    user_entity = UserEntity.create_from_db_data(
        await UserService.get_user(user_entity.id))
    await CardService.create_card(user_id=user_entity.id,
                                  first_name=None,
                                  last_name=None)
    # Response to user
    text_back = "Card was issued. Use /card_info to check your card " \
                "information"
    await context.bot.send_message(chat_id=update.effective_chat.id,
                                   text=text_back)


async def card_info_handler(update: Update,
                            context: ContextTypes.DEFAULT_TYPE):
    """This handler updates user data, checks if this command is available to
    ...
    if not await Checker.has_card(user_entity):
        return await context.bot.send_message(chat_id=update.effective_chat.id,
                                              text=NO_CARD_RESPONSE)
    # Response to user
    card_entity = CardEntity.create_from_db_data(
        await CardService.get_card(user_entity.id))
    account_entity = BankAccountEntity.create_from_db_data(
        await BankAccountService.get_bank_account(user_entity.id))
